...
9. **Is the D-21 spike worth doing at all now?**
10. **Agent-accessible tmux API surface (`lib/toolkit-pane.sh` and beyond).** Expose a library module that lets agents spawn new panes, create sessions, and use the full tmux API programmatically — new-window, split-window, new-session, kill-pane, resize-pane, swap-pane, move-window, and the full option/info surface. This is the agent-facing complement to the human-facing menu system: where `display-menu` serves the human at `prefix+W`, a `tk_pane_*` / `tk_session_*` module serves the agent through its Bash tool. Must live in `lib/toolkit-pane.sh` (separate from `toolkit.sh` — the contract-test `send-keys` exclusion still applies, since the resumer's typing path is the only keystroke injector and remains in the resumer). Priority order: session/pane creation and destruction first (the verbs agents need to manage their own workspace), then info/query, then option management. It only pays off for codex/gemini/pi busy-state, and only if `set-hook -B` turns out to be globally scoped. Defer it until at least one non-Claude harness is actually in daily use, because Claude is covered by the registry and scraping the others buys nothing today.
